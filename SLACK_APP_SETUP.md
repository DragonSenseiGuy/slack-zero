...
**Recommended — self-signed HTTPS on localhost:**

```bash
npm run dev:https
```

This serves the app on `https://localhost:3000`, matching the default
`SLACK_REDIRECT_URI` and `APP_BASE_URL` in `.env`, so nothing else needs
changing. Your browser will warn once about the certificate — click through
(**Advanced → Proceed to localhost**) and continue.

The certificate is generated by `npm run certs` (openssl, run automatically by
`dev:https`, skipped if `certificates/localhost.pem` already exists). Both
`*.pem` and `/certificates/` are gitignored — never commit the key.

> We generate the cert with openssl rather than using bare
> `next dev --experimental-https`, because that flag shells out to `mkcert
> -install`, which needs an interactive sudo password to add a CA to the system
> trust store. When that prompt can't be answered, Next **silently falls back to
> plain HTTP** — the server looks fine but Slack then rejects the redirect URI.
> If you'd rather have a trusted cert with no browser warning, run `mkcert
> -install` once yourself and delete `certificates/` to regenerate.

**Fallback — ngrok**, if localhost HTTPS causes trouble:

> Heads up: some ISP/router content filters (e.g. Xfinity Advanced Security,
> which redirects via `safebrowse.io`) block `*.ngrok-free.dev` outright. The
> symptom is a TLS handshake failure on port 443 and a `302` to a warning page
> on port 80, while `ngrok.com` itself still loads and the ngrok agent reports
> the tunnel healthy. Prefer localhost HTTPS above if you hit that.
